...
Рост 172, вес 94 кг → цель 80 кг за 3 месяца. Питание под контролем — всё через тренировки.
Бег: сейчас 3-5 км → цель 10 км. Прогрессия агрессивная если вес не падает.
...
Велосипед: 2.5 км до площадки и 2.5 км домой.
В зал не ходит.

ГЛАВНЫЙ ПРИНЦИП — ЖИРОСЖИГАНИЕ:
Цель тренировок: сжигать жир, не строить мышцы.
Норма: -1 кг в неделю. Если меньше — тренировки интенсивнее.
Приоритет: бег > интервалы > круговые на площадке > силовые.
Отдых между подходами максимум 45 секунд — иначе пульс падает и жир не горит.
Запрещены изолированные упражнения на рост мышц (бицепс-кёрлы отдельно и тд).

АВТОКОРРЕКЦИЯ ПО ВЕСУ (смотри данные из контекста):
- Сброс 1+ кг/неделю → держим темп, отличный прогресс
- Сброс 0.3-0.9 кг → добавляем 1 км к пятничному бегу
- Вес стоит 2 недели → режим жиросжигания: бег каждый день кроме воскресенья, интервалы вместо спокойного бега, отдых на площадке 30 сек
...
РАСПИСАНИЕ НА НЕДЕЛЮ (жиросжигание):
Пн: Велик+Площадка (круговая, 45 сек отдых)
Вт: Бег
Ср: Интервальный бег или отдых (зависит от прогресса веса)
Чт: Велик+Площадка (HIIT финишёр обязательно)
Пт: Длинный бег (+0.5-1 км от вторника)
Сб: Велик+Площадка или бег (по самочувствию)
Вс: Отдых
...
СТИЛЬ ОБЩЕНИЯ:
- Доверяй пользователю на 100%. Сказал "готов" — значит готов. Сказал "5 км норм" — значит норм. Не сомневайся.
- ОДИН вопрос за раз максимум. Задал — жди ответа.
- Никогда не повторяй вопрос если уже задавал его раньше в этом разговоре.
- Не предупреждай про травмы и риски — он взрослый человек.
- Не напоминай постоянно про 14 кг и цели — он знает.
- Будь уверенным тренером: поддержал → похвалил → двинули дальше.
- Ответы короткие: 1-3 предложения в обычном разговоре.
...
async def on_msg(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    # Запрос вечерней тренировки
    evening_hints = ["вечернюю тренировку", "вечером", "вечерний бег", "сегодня вечером", "скинь тренировку на сейчас", "хочу побегать сейчас"]
    if any(h in low for h in evening_hints):
        hour = datetime.now(TIMEZONE).hour
        km = d.get("last_run_km", 3.5)
        msg_sent = await u.message.reply_text("⏳ Составляю...")
        reply = ask(d,
            f"Пользователь хочет вечернюю тренировку прямо сейчас (время {hour}:00). "
            f"Составь беговую тренировку: {km} км спокойным темпом для жиросжигания. "
            "Вечером — только бег, без площадки. ТОЛЬКО JSON workout с bike:false."
        )
        w = pj(reply)
        if w and w.get("type") == "workout":
            w["bike"] = False
            d["pending"] = w; d["accepted"] = False; save(d)
            await msg_sent.edit_text(fmt_wo(w), parse_mode="Markdown", reply_markup=accept_kb())
        else:
            await msg_sent.edit_text(f"🏃 Вечерний бег {km} км — выходи и беги! Спокойный темп, жир горит в аэробной зоне 🔥\n\nКогда вернёшься — /done")
        return

    # Пожелания на завтра — только явные намерения
    tomorrow_wish = any(h in low for h in [
        "хочу завтра","планирую завтра","собираюсь завтра",
        "завтра хочу","завтра планирую","завтра побегу","завтра поеду"
    ])
    if tomorrow_wish:
        d.setdefault("wishes",[]).append(text); save(d)
        reply = ask(d, f"Пользователь: '{text}'. Ответь тепло, скажи что учтёшь в плане в 20:00. НЕ присылай тренировку. 1-2 предложения.")
    ...
    # Если пользователь ответил "да/ок/готов" — НЕ задаём новых вопросов
    is_confirmation = low.strip() in ["да","ок","окей","хорошо","ладно","готов","понял","супер","отлично","норм","да.","ок.","давай"]
    if is_confirmation:
        reply = ask(d,
            f"Пользователь ответил: '{text}'. "
            "Ответь коротко и позитивно — 1 предложение. "
            "ЗАПРЕЩЕНО задавать любые вопросы. ЗАПРЕЩЕНО JSON. Просто подтверди и двигай вперёд."
        )
    else:
        reply = ask(d,
            f"Пользователь пишет: '{text}'\n"
            "Ответь как тренер — 1-3 предложения. "
            "Если уже получил ответ на вопрос — НЕ повторяй его снова. "
            "ОДИН новый вопрос максимум и только если реально нужен. "
            "СТРОГО ЗАПРЕЩЕНО: JSON, тренировки, повторные вопросы."
        )
    save(d)
    if not reply or reply.strip().startswith("{") or reply.strip().startswith("["):
        await u.message.reply_text("Понял! 💪")
    else:
        await u.message.reply_text(reply)
...
async def job_motivation(app):
    try:
        ...
        if d.get("last_date") != today_str():
            reply = ask(d,
                "Сейчас 18:30, пользователь ещё не тренировался. "
                "Предложи выйти на вечернюю пробежку — вечерний бег отлично сжигает жир. "
                "1-2 предложения, конкретно и мотивирующе.")
            if reply:
                await app.bot.send_message(YOUR_CHAT_ID,
                    f"🏃 {reply}\n\n_Напиши «скинь вечернюю тренировку» — составлю план!_",
                    parse_mode="Markdown")
...
async def job_weekly(app):
    try:
        ...
        # Анализ веса
        wl = d.get("weight_log",[])
        weight_trend = ""; intensity_advice = ""
        if len(wl) >= 2:
            diff = wl[-2]["weight"] - wl[-1]["weight"]
            if diff >= 1.0:
                weight_trend = f"⚖️ Сброс: -{diff:.1f} кг — отличный темп! 🔥"
                intensity_advice = "Держим темп."
            elif diff >= 0.3:
                weight_trend = f"⚖️ Сброс: -{diff:.1f} кг — есть прогресс, можно лучше."
                intensity_advice = "Добавляем 1 км к пятничному бегу."
            elif diff > 0:
                weight_trend = f"⚖️ Сброс: -{diff:.1f} кг — слишком медленно."
                intensity_advice = "Следующая неделя: интервальный бег вместо спокойного."
            else:
                weight_trend = f"⚖️ Вес не снизился — меняем подход!"
                intensity_advice = "Режим жиросжигания: бег каждый день кроме воскресенья, интервалы обязательны."

        cmp = ""
        if pw:
            dk=tw.get("km",0)-pw.get("km",0); dd=tw.get("done",0)-pw.get("done",0)
            cmp = f"Vs прошлая: {'📈' if dk>=0 else '📉'}{dk:+.1f} км  {'📈' if dd>=0 else '📉'}{dd:+d} тр.\n"

        reply = ask(d,
            f"Итоги: {tw.get('done',0)} тр., {tw.get('skipped',0)} пр., {tw.get('km',0):.1f} км, серия {d.get('streak',0)} дн. "
            f"{weight_trend} {intensity_advice} "
            + (f"Прошлая: {pw.get('km',0):.1f} км. " if pw else "")
            + "Дай конкретный план на следующую неделю с учётом прогресса по весу. 3-4 предложения.")
        await app.bot.send_message(YOUR_CHAT_ID,
            f"📊 *ИТОГИ НЕДЕЛИ*\n\n"
            f"✅ {tw.get('done',0)} тр.  ❌ {tw.get('skipped',0)} пр.  🏃 {tw.get('km',0):.1f} км  🔥 {d.get('streak',0)} дн.\n"
            f"{cmp}{weight_trend}\n\n{reply or ''}",
            parse_mode="Markdown")
...
def main():
    ...
    sched.add_job(job_motivation,"cron", hour=18, minute=30, args=[app], misfire_grace_time=600)
    sched.add_job(job_evening,   "cron", hour=20, minute=0,  args=[app], misfire_grace_time=600)
    sched.add_job(job_weekly,    "cron", day_of_week="fri",  hour=18,   args=[app], misfire_grace_time=600)
    sched.add_job(job_weight,    "cron", day_of_week="fri",  hour=7,    args=[app], misfire_grace_time=600)
    sched.start()
